...
CREATE DATABASE syllabus_dev_db;
...
-- dev_user: dev_db 全操作 + syllabus_db 読み取り専用
...
GRANT ALL PRIVILEGES ON DATABASE syllabus_db TO master_user;
GRANT ALL PRIVILEGES ON DATABASE syllabus_dev_db TO master_user;
...
-- app_user/dev_user に syllabus_db への読み取り権限のみ
-- ※ PostgreSQL では DATABASE への GRANT だけではなく、スキーマやテーブルに対する明示的な付与が必要

-- === syllabus_db に対して dev_user/app_user を読み取り専用で設定 ===
...
-- dev_user に syllabus_dev_db の public スキーマ操作権限
GRANT ALL PRIVILEGES ON SCHEMA public TO dev_user;

-- ========== テーブル作成（syllabus_dev_db） ==========
...
-- マイグレーションファイルを実行
\i /docker-entrypoint-initdb.d/migrations/V20250521111859__insert_subject_names.sql
\i /docker-entrypoint-initdb.d/migrations/V20250521111859__insert_class_notes.sql
