...
# 試験用のマイグレーションファイルを取得
$migrationsDir = Join-Path $scriptPath "init/migrations_dev"
...
# マイグレーションを実行
$migrationContent = Get-Content $latestMigration.FullName -Raw -Encoding UTF8
...
# マイグレーションを実行
Get-Content $tempFile -Encoding UTF8 | docker exec -i postgres-db psql -U dev_user -d $devDbName
